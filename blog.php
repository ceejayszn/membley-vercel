            ...
            <?php foreach ($blogs as $blog): ?>
                <div style="background: white; border-radius: 12px; overflow: hidden; box-shadow: var(--shadow-sm); display: flex; flex-direction: column; transition: transform 0.3s ease;">
                    <?php if (!empty($blog['image_url'])): ?>
                        <div style="height: 240px; background-image: url('<?php echo htmlspecialchars(strpos($blog['image_url'], 'http') === 0 ? $blog['image_url'] : '/' . ltrim($blog['image_url'], '/')); ?>'); background-size: cover; background-position: top center;"></div>
                    <?php else: ?>
